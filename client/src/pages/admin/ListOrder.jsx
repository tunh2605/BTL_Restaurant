...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap ${cfg.color}`}
    >
  ...
  return (
    <>
      ...
      >
        {loading ? (
          <>
            <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#C8714A]" />
            <span className="ml-1 text-[#C8714A]">Đang lưu...</span>
          </>
        ) : (
        ...
        {!loading && (
          <ChevronDown
            className={`w-3 h-3 text-[#a08060] transition-transform shrink-0 ${open ? "rotate-180" : ""}`}
          />
        )}
      </button>
...
const ListOrder = () => {
  ...
  const filtered = orders.filter((o) => {
    if (filterMethod !== "all" && o.paymentMethod !== filterMethod)
      return false;
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative" ref={methodRef}>
          ...
          >
            ...
            <ChevronDown
              className={`w-3.5 h-3.5 transition-transform ${methodOpen ? "rotate-180" : ""}`}
            />
          </button>
          {methodOpen && (
              ...
              <tbody>
                {filtered.map((order) => (
                  ...
                  >
                    ...
                    {/* Món */}
                    <td className="px-5 py-4 text-[#7a6050] max-w-50">
                    ...
                    <td className="px-5 py-4 font-semibold text-[#3a2010] whitespace-nowrap">
                      {formatVND(order.finalPrice ?? order.totalPrice ?? 0)}
                    </td>
