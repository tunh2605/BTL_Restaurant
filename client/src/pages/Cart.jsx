...
const Cart = () => {
  ...
  const grouped = cartItems.reduce((acc, item) => {
    ...
    if (!acc[key]) {
      acc[key] = { restaurantId: item.restaurantId, restaurantName: item.restaurantName, items: [] };
    }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (paymentMethod === "vnpay") {
        ...
        const payRes = await axios.post(`${API}/api/payments/create-url`, { orderId });
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 py-32 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-gray-400 mb-8">Bạn chưa thêm món nào vào giỏ hàng.</p>
          <Link
            to="/menu"
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {groupedList.map((group) => (
              <div key={group.restaurantId} className="bg-white rounded-3xl shadow-sm p-6">
                {/* Restaurant header */}
                ...
                <div className="space-y-4">
                  {group.items.map((item) => (
                    <div key={`${item.foodId}-${item.restaurantId}`} className="flex gap-4 items-center">
                      ...
                      <div className="flex items-center gap-1 bg-[#F5EDE3] rounded-full px-1 py-1 shrink-0">
                        <button
                          onClick={() =>
                            updateQuantity(item.foodId, item.restaurantId, item.quantity - 1)
                          }
                      ...
                      <button
                        onClick={() => removeFromCart(item.foodId, item.restaurantId)}
            ...
            <div className="bg-white rounded-3xl shadow-sm p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-gray-700">Ghi chú đơn hàng</span>
              </div>
              <textarea
                value={note}
          ...
          <div className="space-y-4">
            {/* Order summary */}
            <div className="bg-white rounded-3xl shadow-sm p-6">
              <h3 className="font-bold text-gray-800 text-lg mb-4">Tóm tắt đơn hàng</h3>
              <div className="space-y-2 mb-4">
                {cartItems.map((item) => (
                  <div key={`${item.foodId}-summary`} className="flex justify-between text-sm">
            ...
            {/* Payment method */}
            <div className="bg-white rounded-3xl shadow-sm p-6">
              <h3 className="font-bold text-gray-800 text-lg mb-4">Phương thức thanh toán</h3>
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-400">Thẻ ATM / Visa / QR Code</p>
                  </div>
                  <div
